...
export const TrustedBySection: React.FC = () => {
  ...
  return (
    <section className="py-8 sm:py-14 md:py-20 bg-gradient-to-b from-white via-blue-50/20 to-white relative overflow-hidden border-b border-slate-100">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 relative z-10">
        ...
        {/* Header Badge & Title (Compact on Mobile) */}
        <div className="text-center space-y-2 sm:space-y-3 mb-6 sm:mb-10 max-w-2xl mx-auto">
        ...
        {/* Modern Interactive Client Logo Cards (Full Color on Mobile) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-5 max-w-6xl mx-auto">
          {clients.map((client) => (
            <div 
              key={client.id}
              className="group relative bg-white hover:bg-gradient-to-b hover:from-blue-50/50 hover:to-white border border-slate-200/90 hover:border-blue-300 p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl shadow-xs hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col items-center justify-between text-center overflow-hidden"
            >
              ...
              {/* Logo Container On Top (Full Color directly on Mobile) */}
              <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-100/90 flex items-center justify-center p-2 sm:p-3 mb-2 sm:mb-4 group-hover:scale-105 group-hover:bg-blue-50/80 group-hover:border-blue-200 transition-all duration-300 shadow-2xs">
                <img 
                  src={client.logo} 
                  alt={client.name}
              ...
              <div className="space-y-0.5 w-full">
                <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-[#126EFE] transition-colors leading-snug truncate">
                  {client.name}
                </div>
