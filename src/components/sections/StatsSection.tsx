...
const StatCounter: React.FC<StatCounterProps> = React.memo(({ number, suffix, gradientClass }) => {
  ...
  return (
    <div 
      ...
      className={`text-2xl sm:text-4xl md:text-5xl font-black bg-gradient-to-r ${gradientClass || 'from-slate-800 to-slate-900'} bg-clip-text text-transparent tracking-tight`}
    >
      {formattedValue()}
    </div>
  );
...
export const StatsSection: React.FC = () => {
  const stats = [
    { 
      ...
      icon: <FaRocket className="w-4 h-4 sm:w-6 sm:h-6 text-[#126EFE]" />,
    ...
    { 
      ...
      icon: <FaUsers className="w-4 h-4 sm:w-6 sm:h-6 text-[#FBA41C]" />,
    ...
    { 
      ...
      icon: <FaChartLine className="w-4 h-4 sm:w-6 sm:h-6 text-emerald-500" />,
    ...
    { 
      ...
      icon: <FaHeadset className="w-4 h-4 sm:w-6 sm:h-6 text-indigo-600" />,
  ...
  return (
    <section className="relative py-6 sm:py-12 md:py-16 bg-gradient-to-b from-white via-slate-50/50 to-white overflow-hidden border-y border-slate-100">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4 md:gap-6">
          {stats.map((stat, idx) => (
            <div 
              key={idx}
              className="group relative bg-white/90 backdrop-blur-md rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 md:p-6 border border-slate-200/80 hover:border-blue-300 shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-2 sm:space-y-4 overflow-hidden"
            >
              ...
              <div className="relative z-10 flex items-center justify-between gap-1">
                <div className={`p-2 sm:p-3.5 rounded-xl sm:rounded-2xl border ${stat.bgIcon} group-hover:scale-105 transition-transform duration-300 shadow-2xs`}>
                  {stat.icon}
                </div>
                <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full border ${stat.badgeClass} flex items-center gap-0.5 sm:gap-1 shrink-0`}>
                  <FaCheck className="w-2 h-2 sm:w-2.5 sm:h-2.5" />
                  {stat.badge}
                </span>
              </div>

              <div className="relative z-10 space-y-0.5 sm:space-y-1 pt-1 sm:pt-2">
                <StatCounter number={stat.number} suffix={stat.suffix} gradientClass={stat.gradientClass} />
                <div className="text-xs sm:text-sm font-bold text-slate-800 tracking-tight leading-snug">
                  {stat.label}
                </div>
